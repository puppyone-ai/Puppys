...
def holder_decisiontree(self):
    ...
    while self.check("是否抓到了鬼") == False:
        ...
        for player in self.player_list:
            self.do("让 list中的玩家讨论一下谁是 ghost")
            self.wait("玩家描述完毕")
...
hacker_news.actionflow=[{action:"去 hacker_news网页，保存他们的 HTML"}]
...
def hacker_news(self):
    ...
    while self.check("保存下来了 HTML")==False:
        self.do("去 hacker_news网页，保存他们的 HTML")

    self.do("把前 10 条消息显示出来")

    self.wait_until("当前时间为早上八点")
    self.do("把消息发给我")
