...
INFO_HEIGHT = 100  # Additional height for the information display
FPS = 30
...
def draw_grid(grid, agent_x, agent_y, message):
    """ Draw the grid and the message on the screen. """
    screen.fill(BLACK)
    ...
    action_rect = pygame.Rect(0, SCREEN_HEIGHT, SCREEN_WIDTH, INFO_HEIGHT)
    pygame.draw.rect(screen, LIGHT_BLUE, action_rect)
    action_font = pygame.font.Font(None, 20)
    action_text = action_font.render(message, True, BLACK)
...
current_action = "Welcome to the Room Escape Game!"
        ...
        if event.type == QUIT:
            ...
            if event.key in (K_UP, K_w) and agent_y > 0:
                if grid[agent_y - 1][agent_x] == ' ':
                    grid[agent_y][agent_x], grid[agent_y - 1][agent_x] = ' ', 'A'
                    agent_y -= 1
                    current_action = "The agent is now moving up."
                elif grid[agent_y - 1][agent_x] == 'T':
                    current_action = "The agent is moving up, taking the tool."
            ...
            # Move down
                ...
                if grid[agent_y + 1][agent_x] == ' ':
                    grid[agent_y][agent_x], grid[agent_y + 1][agent_x] = ' ', 'A'
                    agent_y += 1
                    current_action = "The agent is now moving down."
                elif grid[agent_y + 1][agent_x] == 'T':
                    current_action = "The agent is moving down, taking the tool."

            # Move left
            elif event.key in (K_LEFT, K_a) and agent_x > 0:
                if grid[agent_y][agent_x - 1] == ' ':
                    grid[agent_y][agent_x], grid[agent_y][agent_x - 1] = ' ', 'A'
                    agent_x -= 1
                    current_action = "The agent is now moving left."
                elif grid[agent_y][agent_x - 1] == 'T':
                    current_action = "The agent is moving left, taking the tool."
            ...
            # Move right
                ...
                if grid[agent_y][agent_x + 1] == ' ':
                    grid[agent_y][agent_x], grid[agent_y][agent_x + 1] = ' ', 'A'
                    agent_x += 1
                    current_action = "The agent is now moving right."
                elif grid[agent_y][agent_x + 1] == 'T':
                    current_action = "The agent is moving right, taking the tool."

            # Add additional keys for interactions with doors or other items

    draw_grid(grid, agent_x, agent_y, current_action)
    pygame.display.flip()
    clock.tick(FPS)
