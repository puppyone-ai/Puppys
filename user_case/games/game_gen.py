...
# from puppys.environment.chatting import ChattingHistory
from puppys.llm.open_ai import open_ai_chat


background = """
在中世纪的一个小镇上，巫术和神秘力量一直在暗中流传。统治者利用猎巫行动，抓捕了“大巫女”和她的部分信徒，实际上大巫女是小镇最有名的医师，
...
def game_gen(
    ...
    while num < 5:
        system_prompt = (
            [
                {"role": "system",
                 "content": f"""你是一个游戏的上帝，游戏背景：{background}，现在我们要给玩家设置一个任务，这个任务越离谱越魔幻越好，生成的任务最好和之前的任务有一定程度的设定和事物的关联，但是不要完全相同或者相似。这个任务有一个完成的条件，完成之后用户可以获得奖励，并且引导用户一步一步使得城镇越来越繁荣。
                       ...
                       "condition": {{"wood": 100, "food": 50}}
                       "reward": {{"gold": 100, "wood": 10}}}}
                       """
                }
            ]
        )
