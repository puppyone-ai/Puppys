...
"""
    ...
    assert res[0].do.strip() == '## go to https://news.ycombinator.com/ show the HTML\nhacker_news.do()'
    assert res[1].do.strip() == '## show the top 10 news @gpt, and send it to me\nhacker_news.do()'
    assert res[2].do.strip() == '## pick the news that related to Large Language Models, summarize all the news, and show it to me\nhacker_news.do()'
